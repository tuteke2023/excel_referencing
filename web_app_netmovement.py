...
class TBGLLinkerNetMovementWeb:
...
def main():
    st.set_page_config(
        page_title="TB-GL Linker (Net Movement)",
        page_icon="📊",
        layout="wide"
    )

    st.title("📊 TB-GL Linker (Net Movement Version)")
    st.markdown("**Link your Trial Balance to General Ledger Net Movement figures**")
    st.info("This version links to Net Movement cells and displays the actual value as the hyperlink text.")
    ...
    with st.expander("🔄 How This Version Differs"):
        st.markdown("""
        **Original Version:**
        - Links to account header row (e.g., `A9829`)
        - Displays "View Details" as link text
    ...
    if tb_file and gl_file:
        ...
        with col2:
            output_filename = st.text_input(
                "Output Filename",
                value="TB_GL_Linked_NetMovement.xlsx",
        ...
        if st.button("🚀 Process Files", type="primary"):
            try:
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    linker = TBGLLinkerNetMovementWeb(tb_path, gl_path, output_path)
